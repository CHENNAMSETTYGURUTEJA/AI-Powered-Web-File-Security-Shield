...
async function sendPing() {
  try {
    ...
    // Determine which URL to use (we try production first, then fallback to local if it fails significantly)
    // For this fix, we will just use the one defined in configuration
    const url = `${API_BASE_URL}/api/extension/ping`;
    
    console.log(`[HEARTBEAT] Sending ping | Client: ${currentClientId} | URL: ${url}`);
    ...
    console.error(`[HEARTBEAT] Network Error:`, error.message);
  }
}
...
chrome.runtime.onInstalled.addListener(() => {
  ...
  // Create alarm for pings (exactly every 5 seconds as per requirements)
