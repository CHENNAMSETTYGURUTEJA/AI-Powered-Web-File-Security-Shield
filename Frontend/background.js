...
chrome.downloads.onDeterminingFilename.addListener((item, suggest) => {
  console.log("Detecting download:", item);

  const fileData = {
    filename: item.filename,
    file_size_bytes: item.fileSize || 0, // Handle unknown size
    file_type: item.mime || "unknown"
  };

  fetch("http://127.0.0.1:8000/scan_file", {
    method: "POST",
    ...
    .then(data => {
      ...
      // Check if Malicious (0 = Malitious, 1 = Safe)
      if (data.result === "Malicious" || data.prediction === 0) {
        // BLOCK DOWNLOAD
        console.log("Blocking malicious file:", item.filename);
        ...
        // Open Blocked Page with filename
        const blockedUrl = chrome.runtime.getURL(`blocked.html?file=${encodeURIComponent(item.filename)}`);
        chrome.tabs.create({ url: blockedUrl });

        // Show BLOCK Notification (Backup)
        chrome.notifications.create({
          type: 'basic',
          iconUrl: 'icons/icon48.png',
          title: '🚨 MALICIOUS FILE BLOCKED!',
          message: `PhishShield blocked "${item.filename}".`,
          priority: 2
        });

      } else {
        // ALLOW DOWNLOAD
        console.log("Allowing safe file:", item.filename);
        ...
        chrome.notifications.create({
          ...
          iconUrl: 'icons/icon48.png',
          title: '✅ File Safe',
          message: `PhishShield scanned "${item.filename}" and found it safe.`,
          priority: 1
        });
      }
    })
    .catch(err => {
      ...
      chrome.notifications.create({
        type: 'basic',
        iconUrl: 'icons/icon48.png',
        title: '⚠️ Scan Failed - Backend Offline?',
        message: `PhishShield could not reach the server.\nFile: "${item.filename}"\nAllowed download for now.`,
        priority: 2
      });
  ...
  return true; // Indicates we are responding asynchronously
}); 